...
if(isset($_GET["option"])&&$_GET["option"] == "deact"){
  ...
  if(mysqli_connect_errno()){
    ...
    $sql = "SELECT deactivated FROM tblauteur WHERE auteurID = $deactID";
  ...
  if(mysqli_connect_errno()){
    ...
    if($deact == 0){
      $sql = "UPDATE tblauteur SET deactivated = 1 WHERE auteurID = $deactID";
    }
    else{
      $sql = "UPDATE tblauteur SET deactivated = 0 WHERE auteurID = $deactID";
    }
    if($stmt = $mysqli->prepare($sql)){
      if(!$stmt->execute()){
